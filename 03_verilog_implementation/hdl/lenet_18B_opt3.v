module lenet (
    ...
    //    COMPUTE Circuits
    //=====================================
    wire signed [7:0]  mul_A[0:39];
    wire signed [7:0]  mul_B[0:39];
    wire signed [15:0] mul_O[0:39];
    wire signed [17:0] mac_A[0:7];
    wire signed [17:0] mac_B[0:7];
    reg  signed [17:0] mac_reg[0:7];
    wire signed [17:0] acc_O;
    wire signed [17:0] acc_fc_sum;
    ...
    always@(posedge clk)begin
        case(cur_state)
            ...
            FC2:begin
                if(timer == 14) begin
                    sram_act_wdata0  <= acc_fc_sum + bias_buf;
                end
            end
        endcase
    end
    ...
    generate
        genvar gen_mul_i;
        for(gen_mul_i=0; gen_mul_i<40; gen_mul_i=gen_mul_i+1)begin:gen_muls
            MUL_8X8 mul_8x8(mul_A[gen_mul_i],mul_B[gen_mul_i],mul_O[gen_mul_i]);
        end
    endgenerate

    
    assign mul_A[0] = weight_buf_wire_1[0];
    assign mul_B[0] = act_buf_wire_1[0]   ;
    assign mul_A[1] = weight_buf_wire_1[1];
    assign mul_B[1] = act_buf_wire_1[1]   ;
    assign mul_A[2] = weight_buf_wire_1[2];
    assign mul_B[2] = act_buf_wire_1[2]   ;
    assign mul_A[3] = weight_buf_wire_1[3];
    assign mul_B[3] = act_buf_wire_1[3]   ;
    assign mul_A[4] = weight_buf_wire_1[4];
    assign mul_B[4] = act_buf_wire_1[4]   ;
    assign mul_A[5] = (cur_state < 5) ? weight_buf_wire_1[0] : weight_buf_wire_1[5] ;
    assign mul_B[5] = (cur_state < 5) ? act_buf_wire_1[1]    : act_buf_wire_1[5]    ;
    assign mul_A[6] = (cur_state < 5) ? weight_buf_wire_1[1] : weight_buf_wire_1[6] ;
    assign mul_B[6] = (cur_state < 5) ? act_buf_wire_1[2]    : act_buf_wire_1[6]    ;
    assign mul_A[7] = (cur_state < 5) ? weight_buf_wire_1[2] : weight_buf_wire_1[7] ;
    assign mul_B[7] = (cur_state < 5) ? act_buf_wire_1[3]    : act_buf_wire_1[7]    ;
    assign mul_A[8] = weight_buf_wire_1[3];
    assign mul_B[8] = act_buf_wire_1[4]   ;
    assign mul_A[9] = weight_buf_wire_1[4];
    assign mul_B[9] = act_buf_wire_1[5]   ;
    // connect weight & act buffer to mac input ports
    generate
        genvar j;
        // assign mul_A,B
        for(i=2;i<4;i=i+1)begin:gen_mul_AB_wire_2
            for(j=0;j<5;j=j+1)begin:gen_mul_AB_wire_1
                assign mul_A[i*5+j] = weight_buf_wire_1[j];
                assign mul_B[i*5+j] = act_buf_wire_1[i+j];
            end
        end
        for(i=0;i<4;i=i+1)begin:gen_mul_AB_wire_4
            for(j=0;j<5;j=j+1)begin:gen_mul_AB_wire_3
                assign mul_A[i*5+j+20] = weight_buf_wire_2[j];
                assign mul_B[i*5+j+20] = act_buf_wire_2[i+j];
            end
        end
    endgenerate
    ...
    generate
        for(i=0;i<1;i=i+1)begin:gen_mac_reg_1
            always@(posedge clk)begin
                if(!rst_n) mac_reg[i] <= 0;
                    ...
                    case(cur_state)
                        CONV1:begin
                            case(timer)
                                0:mac_reg[i] <= 0;
                                2,3,4,5,6:mac_reg[i] <= conv_sum[i];
                            endcase
                        end
                        CONV2:begin
                            case(timer)
                        ...
                        CONV3:begin
                            if((timer > 1) && (timer < 52)) mac_reg[i] <= acc_O;
                            else if(timer == 0) mac_reg[i] <= 0;
                        end
                        FC1:begin
                            if((timer > 1) && (timer < 17)) mac_reg[i] <= acc_O;
                            else if(timer == 0) mac_reg[i] <= 0;
                        end
                        FC2:begin
                            if((timer > 1) && (timer < 12))mac_reg[i] <= acc_O;
                            else if(timer == 0) mac_reg[i] <= 0;
                        end
                        OUTPUT:mac_reg[i] <= 0;
                    endcase
        ...
        for(i=1;i<4;i=i+1)begin:gen_mac_reg_2
            always@(posedge clk)begin
                if(!rst_n) mac_reg[i] <= 0;
                else begin
                    case(cur_state)
        ...
        for(i=4;i<8;i=i+1)begin:gen_mac_reg_3
            always@(posedge clk)begin
                if(!rst_n) mac_reg[i] <= 0;
                else begin
                    case(cur_state)
                        CONV1:begin
                            case(timer)
                                0:mac_reg[i] <= 0;
                                3,4,5,6,7:mac_reg[i] <= conv_sum[i];
                            endcase
                        end
                        CONV2:begin
                            case(timer)
    ...
    generate
        for (i=0;i<8;i=i+1)begin:gen_conv_sum
            assign conv_sum[i] = ((mul_O[i*5] + mul_O[i*5+1]) + (mul_O[i*5+2] + mac_reg[i])) + (mul_O[i*5+3] + mul_O[i*5+4]);
        end
    endgenerate
    

    ACC acc(fc_sum, mac_reg[0], acc_O);

    assign fc_sum = ((mul_O[0]+mul_O[1]) + (mul_O[2]+mul_O[3])) + ((mul_O[4]+mul_O[5]) + (mul_O[6]+mul_O[7]));
    assign acc_fc_sum = mac_reg[0];
    ...
    generate
        for (i=0;i<2;i=i+1)begin:gen_pool
            POOL pool(mac_reg[i*2],mac_reg[i*2+1],mac_reg[i*2+4],mac_reg[i*2+5],pool_out[i]);
        end
    endgenerate
    ...
    assign quantized_pool_out[1] = pool_out[1]*scale_factor[13:0];
    ...
    assign relued_fc_sum = (acc_fc_sum < 0) ? 0 : acc_fc_sum;
    assign quantized_relued_fc_sum = mo;
    assign shifted_reluwd_fc_sum = quantized_relued_fc_sum[31:16];
